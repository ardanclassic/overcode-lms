...
import { CheckSquare, Users, AlertTriangle, Coins, CheckCircle2 } from "lucide-react";
...
export default function TeacherDashboard() {
  ...
  // States
  const [tokens] = useState(12);
  const [isTopUpModalOpen, setIsTopUpModalOpen] = useState(false);
  const [topUpAmount, setTopUpAmount] = useState("");
  const [topUpSuccess, setTopUpSuccess] = useState(false);

  const [targets, setTargets] = useState([
    { id: 1, text: "Menjelaskan analogi brankas untuk Database.", done: false },
    { id: 2, text: "Praktik membuat project Supabase baru.", done: false },
    { id: 3, text: "Mengamankan kredensial database di file .env.", done: false },
  ]);

  const [roster, setRoster] = useState([
    { id: "S1", name: "Budi Santoso", progress: 50, status: "locked" },
    { id: "S2", name: "Andi Wijaya", progress: 50, status: "locked" },
    { id: "S3", name: "Siti Rahma", progress: 50, status: "locked" },
  ]);
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto pb-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        <FadeIn className="glass p-5 rounded-2xl flex flex-col justify-center gap-1 shadow-sm md:col-span-2 relative overflow-hidden group">
          ...
          <div className="flex justify-between items-center w-full relative z-10">
            <div>
              <div className="flex items-center gap-3 mb-1">
                ...
                <h2 className="text-2xl font-black text-slate-800 tracking-tight">OverCode Batch 5</h2>
              </div>
              <p className="text-sm font-medium text-slate-500 ml-5.5">Sesi Aktif: Pertemuan 5 - Supabase Database</p>
            </div>
            <div className="flex items-center gap-4 bg-white border border-slate-200 px-5 py-2.5 rounded-xl hidden sm:flex shadow-sm">
              <span className="font-mono text-2xl font-black text-primary">85:00</span>
              <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">tersisa</span>
            </div>
          </div>
        </FadeIn>
